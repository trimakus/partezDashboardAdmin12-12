...
class RctCollapsibleCard2 extends Component {
        ...
        return (
            <div className={classnames(colClasses ? colClasses : '', { 'd-block': !collapse }) }>
                <div className={classnames(`rct-block ${customClasses ? customClasses : ''}`, { 'd-none': close })} >
                    {heading &&
                    <div className={`rct-block-title ${headingCustomClasses ? headingCustomClasses : ''}`} style={{background:"rgba(93, 146, 242, 1)"}}>
                        <h2 style={{color:"#FFF",fontWeight:"700"}}>{heading} </h2>
                        {(collapsible || reloadable || closeable) &&
                        <div className="contextual-link">
                            {collapsible && <a href="javascript:void(0)" onClick={() => this.onCollapse()}><i className="ti-minus" style={{color:"#FFF"}}></i></a>}
...
export default RctCollapsibleCard2;
